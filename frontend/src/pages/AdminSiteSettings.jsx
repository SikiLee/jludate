...
function normalizeEmailTemplates(rawValue) {
  ...
  const exceptionRejectedBody = typeof exceptionRejected.body === 'string' ? exceptionRejected.body.replace(/\r\n/g, '\n').trim() : '';
...
function AdminSiteSettings() {
  ...
  const applyPayload = (payload) => {
    const brandName = payload?.brand_name || 'JLUDate';
  ...
  const handleSave = async () => {
    ...
    if (!emailTemplates.exception_approved?.subject || !emailTemplates.exception_approved?.body) {
      toast.error('请完整填写异常邮箱核验通过通知模板');
      return;
    }
    if (!emailTemplates.exception_rejected?.subject || !emailTemplates.exception_rejected?.body) {
      toast.error('请完整填写异常邮箱核验驳回通知模板');
      return;
    }
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="space-y-6">
        ...
        <ConfigSection title="邮件模板设置" defaultOpen={false}>
          <section className="space-y-4">
            ...
            <div className="p-4 rounded-2xl border border-slate-200 bg-slate-50/70 space-y-3">
              ...
              <input
                type="text"
                value={form.email_templates?.exception_approved?.subject || ''}
                onChange={(e) => handleEmailTemplateChange('exception_approved', 'subject', e.target.value)}
                placeholder="邮件标题"
                className="w-full px-3 py-2.5 rounded-xl border border-slate-200 bg-white focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none transition"
              />
              <textarea
                value={form.email_templates?.exception_approved?.body || ''}
                onChange={(e) => handleEmailTemplateChange('exception_approved', 'body', e.target.value)}
                placeholder="邮件正文"
                className="w-full min-h-[140px] px-3 py-2.5 rounded-xl border border-slate-200 bg-white focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none transition font-mono text-sm"
              />
            </div>
            ...
            <div className="p-4 rounded-2xl border border-slate-200 bg-slate-50/70 space-y-3">
              <h3 className="text-sm font-bold text-slate-700">异常邮箱核验驳回通知</h3>
              <input
                type="text"
                value={form.email_templates?.exception_rejected?.subject || ''}
                onChange={(e) => handleEmailTemplateChange('exception_rejected', 'subject', e.target.value)}
                placeholder="邮件标题"
                className="w-full px-3 py-2.5 rounded-xl border border-slate-200 bg-white focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none transition"
              />
              <textarea
                value={form.email_templates?.exception_rejected?.body || ''}
                onChange={(e) => handleEmailTemplateChange('exception_rejected', 'body', e.target.value)}
                placeholder="邮件正文"
                className="w-full min-h-[140px] px-3 py-2.5 rounded-xl border border-slate-200 bg-white focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none transition font-mono text-sm"
              />
            </div>
        ...
        <ConfigSection title="首页“为什么选择我们”" defaultOpen={false}>
          ...
          <p className="text-xs text-slate-500 mb-3">支持变量：`{'{XXDate}'}`、`{'{ALLOWED_DOMAINS}'}`、`{'{MATCH_REVEAL_AT}'}`（会随“自动配对时间”联动）</p>
