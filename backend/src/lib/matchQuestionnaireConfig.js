...
const QUESTIONNAIRE_TYPES = new Set(['love', 'friend', 'xinghua']);
...
function getXinghuaHardSettingsItems() {
  const base = getDefaultHardSettingsItems();
  const withoutSmoking = base.filter((item) => item.options_json?.payload_key !== 'accept_smoking');
  const hardPart = withoutSmoking.filter((item) => item.page_key === 'hard');
  const settingsPart = withoutSmoking.filter((item) => item.page_key === 'settings');

  hardPart.push({
    page_key: 'hard',
    question_number: 5,
    question_kind: 'select',
    question_title: '你更倾向什么时候去？',
    question_stem: '',
    left_option_text: '',
    right_option_text: '',
    options_json: {
      payload_key: 'preferred_time',
      placeholder: '请选择',
      choices: [
        { value: 'sun_am', label: '周日上午' },
        { value: 'sun_pm', label: '周日下午' },
        { value: 'any', label: '都行' }
      ]
    }
  });

  return [...hardPart, ...settingsPart];
}
...
export async function seedXinghuaQuestionnaireConfigIfMissing(db) {
  ...
  await db.query('BEGIN');
  try {
    for (const item of hardSettingsDefaults) {
      await db.query(
        `
        INSERT INTO unidate_app.match_questionnaire_items(
          questionnaire_type,
    ...
    await db.query('COMMIT');
    return { seeded: true, count: hardSettingsDefaults.length };
  } catch (e) {
    await db.query('ROLLBACK');
    throw e;
  }
