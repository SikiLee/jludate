...
async function resolveUserFromBearerToken(token) {
  ...
  if (/^\d+$/.test(subject)) {
    result = await identityPool.query(
      'SELECT id, is_admin, is_active FROM unidate_app.users WHERE id = $1 LIMIT 1',
      [Number(subject)]
    );
  } else {
    // Legacy compatibility for old tokens where sub was email.
    result = await identityPool.query(
      'SELECT id, is_admin, is_active FROM unidate_app.users WHERE email = $1 LIMIT 1',
      [subject]
    );
  }
  ...
  if (!user.is_active) {
    return { error: { status: 401, msg: 'Account disabled' } };
  }
