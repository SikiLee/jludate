...
async function writeIdentityAuditLog(db, { actor, action, targetType, targetRef, purpose }) {
  try {
    await db.query(
      `
      INSERT INTO szudate_app.access_audit_logs(actor, action, target_type, target_ref, purpose)
      VALUES ($1, $2, $3, $4, $5)
      `,
      [
        safeActor(actor),
...
export async function getRespondentIdByUserId(db, userId, { actor = 'system', purpose = '' } = {}) {
  const result = await db.query(
    ...
    FROM szudate_app.user_respondent_links
    WHERE user_id = $1
    LIMIT 1
    `,
    [userId]
...
export async function ensureRespondentIdForUser(db, userId, { actor = 'system', purpose = '' } = {}) {
  ...
  for (let attempt = 0; attempt < RESPONDENT_ID_RETRY_LIMIT; attempt += 1) {
    ...
    try {
      await db.query(
        `
        INSERT INTO szudate_app.user_respondent_links(
          user_id,
        ...
        ON CONFLICT (user_id)
        DO UPDATE SET
          respondent_id_ciphertext = szudate_app.user_respondent_links.respondent_id_ciphertext,
          respondent_id_hash = szudate_app.user_respondent_links.respondent_id_hash,
          respondent_id_key_version = szudate_app.user_respondent_links.respondent_id_key_version,
          updated_at = szudate_app.user_respondent_links.updated_at
        `,
...
export async function getUserEmailByRespondentId(db, respondentIdInput, { actor = 'system', purpose = '' } = {}) {
  ...
  const result = await db.query(
    ...
    SELECT
      ...
      u.email_ciphertext,
      u.email_key_version
    FROM szudate_app.user_respondent_links l
    INNER JOIN szudate_app.users u ON u.id = l.user_id
...
export async function listActiveIdentityProfilesForMatching(db, { actor = 'system', purpose = '' } = {}) {
  const result = await db.query(
    ...
    SELECT
      ...
      l.respondent_id_ciphertext,
      l.respondent_id_key_version
    FROM szudate_app.users u
    INNER JOIN szudate_app.user_respondent_links l ON l.user_id = u.id
