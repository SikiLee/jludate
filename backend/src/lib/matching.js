...
export async function runMatchingEngine({ runType = 'manual', runKey: customRunKey, initiatedBy = 'system' } = {}) {
  ...
  const existingRun = await surveyPool.query(
    'SELECT id, status FROM szudate_app.match_runs WHERE run_key = $1 LIMIT 1',
    [runKey]
  );
  ...
  try {
    ...
    const runInsert = await client.query(
      `
      INSERT INTO szudate_app.match_runs(run_type, run_key, status, initiated_by)
      VALUES ($1, $2, $3, $4)
      RETURNING id
      `,
    ...
    for (const pair of selectedPairs) {
      ...
      const insertResult = await client.query(
        `
        INSERT INTO szudate_app.match_results(
          run_id,
          respondent1_id,
          respondent2_id,
          base_match_percent,
