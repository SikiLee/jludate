...
export async function seedSurveyQuestionsIfEmpty(db) {
  const countResult = await db.query('SELECT COUNT(*)::int AS count FROM szudate_app.survey_questions');
  ...
  for (const item of definitions) {
    await db.query(
      `
      INSERT INTO szudate_app.survey_questions(
        question_number,
        section_title,
        question_text,
        display_order
...
export async function importSurveyQuestionsFromDefaults(db, { overwrite = false } = {}) {
  const definitions = getQuestionDefinitionList();
  const existingResult = await db.query('SELECT question_number FROM szudate_app.survey_questions');
...
export async function listSurveyQuestions(db) {
  const result = await db.query(
    ...
    SELECT
      ...
      updated_at,
      updated_by
    FROM szudate_app.survey_questions
