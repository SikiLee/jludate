...
export const XINGHUA_PREFERRED_TIMES = ['sun_am', 'sun_pm', 'any'];
...
function normalizeXinghuaPreferredTime(value) {
  if (typeof value !== 'string') {
    return '';
  }
  const v = value.trim().toLowerCase();
  return XINGHUA_PREFERRED_TIMES.includes(v) ? v : '';
}

function isXinghuaPreferredTimeCompatible(userA, userB) {
  const a = normalizeXinghuaPreferredTime(userA?.xinghua_preferred_time);
  const b = normalizeXinghuaPreferredTime(userB?.xinghua_preferred_time);
  if (!a && !b) {
    return true;
  }
  if (!a || !b) {
...
function resolveRequiredPartnerXinghuaTi(holder) {
  const pref = typeof holder?.xinghua_match_target_ti === 'string'
    ? holder.xinghua_match_target_ti.trim()
    : 'same_as_me';
  if (pref === 'any') {
    return null;
  }
  if (pref === 'same_as_me') {
    const self = typeof holder?.xinghua_ti_type === 'string' ? holder.xinghua_ti_type.trim() : '';
    if (self.length !== 4 || !XINGHUA_TI_TYPE_CODE_SET.has(self)) {
      return null;
    }
    return self;
  }
  if (pref.length === 4 && XINGHUA_TI_TYPE_CODE_SET.has(pref)) {
    return pref;
  }
  return null;
}

function partnerMeetsXinghuaTiPreference(holder, partner) {
  const required = resolveRequiredPartnerXinghuaTi(holder);
  if (!required) {
    return true;
  }
  const pt = typeof partner?.xinghua_ti_type === 'string' ? partner.xinghua_ti_type.trim() : '';
  if (pt.length !== 4 || !XINGHUA_TI_TYPE_CODE_SET.has(pt)) {
    return false;
  }
  return pt === required;
}

function isXinghuaTiPairCompatible(userA, userB) {
  const aFest = userA?.xinghua_festival_participate === true;
  const bFest = userB?.xinghua_festival_participate === true;
  if (!aFest || !bFest) {
    return true;
  }
  return partnerMeetsXinghuaTiPreference(userA, userB) && partnerMeetsXinghuaTiPreference(userB, userA);
}
...
export function evaluateHardFilters(userA, userB) {
  ...
  if (!isXinghuaPreferredTimeCompatible(userA, userB)) {
    return { passed: false, reason: 'xinghua_time_mismatch' };
  }

  if (!isXinghuaTiPairCompatible(userA, userB)) {
    return { passed: false, reason: 'xinghua_ti_mismatch' };
  }
