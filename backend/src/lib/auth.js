...
const SECRET_KEY = process.env.SECRET_KEY || 'supersecretkey-szudate';
...
export async function getCurrentUserFromRequest(request) {
  ...
  if (/^\d+$/.test(subject)) {
    result = await identityPool.query('SELECT id, is_admin FROM szudate_app.users WHERE id = $1 LIMIT 1', [Number(subject)]);
  } else {
    // Legacy compatibility for old tokens where sub was email.
    result = await identityPool.query('SELECT id, is_admin FROM szudate_app.users WHERE email = $1 LIMIT 1', [subject]);
  }
