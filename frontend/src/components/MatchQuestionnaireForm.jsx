...
export default function MatchQuestionnaireForm({ questionnaireType }) {
  ...
  const renderHardQuestion = (item) => {
    ...
    if (kind === 'range') {
      const min = Number.isInteger(Number(item.options_json?.min)) ? Number(item.options_json.min) : 0;
      const max = Number.isInteger(Number(item.options_json?.max)) ? Number(item.options_json.max) : 12;
      const step = Number.isInteger(Number(item.options_json?.step)) ? Number(item.options_json.step) : 1;
      const prefix = typeof item.options_json?.prefix === 'string' ? item.options_json.prefix : '';
      const valueRaw = payload.hard_filter?.[key];
      const value = Number.isInteger(valueRaw) ? valueRaw : min;
      return (
        <div className="space-y-4" key={`hard-${item.id}`}>
          <div>
            ...
            <div className="rounded-2xl border-2 border-roseTint/40 bg-white px-5 py-4">
              <div className="text-sm font-bold text-szured mb-2 font-shsans">
                当前：{prefix}{value}
              </div>
              <input
                type="range"
                ...
                className="w-full accent-[#b65f7b]"
              />
              <div className="flex justify-between text-xs text-[#7a7278] mt-1 font-shsans">
