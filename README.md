# JLUDate
受斯坦福大学 DateDrop 和上海交大 SJTUDate 的启发，我们打造了“JLUDate”。
...
| 服务 | 端口 | 地址 | 说明 |
| --- | --- | --- | --- |
| Frontend | `8080` | [http://localhost:8080](http://localhost:8080) | 用户端页面（注册、问卷、匹配结果） |
...
## Cloudflare + 香港服务器上线（推荐）

1. 服务器开放 `80/tcp`、`443/tcp`（如需直连后端调试再额外开放 `8000/tcp`）。
2. 根目录 `.env` 建议：
   - `WEB_BASE_URL=https://你的域名`
   - `FRONTEND_PORT=8080`（让宿主机 Nginx 占用 `80/443`，避免和容器冲突）
   - `NPM_REGISTRY=https://registry.npmjs.org/`
   - `FORCE_HTTPS=false`（先由 Cloudflare 做 HTTPS，稳定后再切源站强制 HTTPS）
3. Cloudflare DNS：
   - `A` 记录指向香港服务器公网 IP；
   - 代理状态建议先开橙云（Proxied）。
4. Cloudflare SSL/TLS：
   - 先用 `Full`；
   - 完成源站证书部署后切 `Full (strict)`。
5. 启动：
   - `docker compose up --build -d`
   - 访问 `https://你的域名` 验证首页和 `/api/public/site-settings`。
6. 若已配置宿主机 Nginx 反代，请将 `proxy_pass` 指向 `http://127.0.0.1:8080`。
...
1. 打开前端页面 `http://localhost:8080`，进入注册页。
...
5. 管理员账号登录后访问：
   - 前端管理页：`http://localhost:8080/admin`
6. 管理接口（需管理员 Token）：
